    ...
    <div style={{
      ...
      <svg width="60" height="20" viewBox="0 0 60 20" fill="none">
        <rect 
          ...
          rx="1" 
          fill="white" 
          style={{ transition: 'all 0.1s ease-out', transform: isConfused ? 'rotate(-10deg)' : 'none' }} 
        />
        <rect 
          x="38" 
      ...
      <style>{` 
        ...
        .zzz { position: absolute; animation: zzz-float 3s infinite; color: #0a84ff; font-weight: bold; font-size: 12px; }
      `}</style>
    </div>
  );
}
...
function VideoStage({ onDisconnect }: { onDisconnect: () => void }) {
  ...
  // Logic to reset sleep timer on any interaction
  const pokeMalvin = () => {
    setIsSleeping(false);
...
// --- STYLES ---
const zzzStyle: React.CSSProperties = {}; // Placeholders for the CSS above
