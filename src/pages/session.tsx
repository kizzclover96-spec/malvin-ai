...
// --- NEON ICONS (Optimized for tight hitboxes) ---
const GearIcon = () => (
...
function MalvinVoiceIsland({ agent }: { agent: any }) {
  ...
  return (
    <div style={{
      ...
      alignItems: 'center', justifyContent: 'center', boxShadow: isAgentSpeaking ? `0 0 15px ${neonBlue}55` : `0 0 5px ${neonBlue}22`,
    }}>
...
function VideoStage({ onDisconnect }: { onDisconnect: () => void }) {
  ...
  const btnReset: CSSProperties = {
    ...
    width: '24px', height: '24px' // Restricts hit area to icon size
  };
    ...
    <div style={{ position: 'fixed', inset: 0, backgroundColor: '#000', color: '#fff', overflow: 'hidden', fontFamily: 'sans-serif' }}>
      ...
      <div style={{ position: 'absolute', bottom: '30px', left: 0, right: 0, display: 'flex', justifyContent: 'center', zIndex: 100 }}>
        <div style={{
          ...
          {/* KILL SWITCH - Wrapped in a red ring as requested */}
          <button onClick={onDisconnect} style={{
            ...btnReset, width: '30px', height: '30px', borderRadius: '50%', border: `1.5px solid ${neonRed}`,
            color: neonRed, fontSize: '14px', fontWeight: 'bold', marginRight: '12px'
          }}>✕</button>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '14px', marginLeft: '10px' }}>
            <button onClick={() => fileInputRef.current?.click()} style={btnReset}><ClipIcon /></button>
            <input type="file" ref={fileInputRef} style={{ display: 'none' }} />
            
            <button onClick={() => localParticipant?.setCameraEnabled(!localParticipant.isCameraEnabled)} style={btnReset}>
              <CameraIcon enabled={!!localParticipant?.isCameraEnabled} />
            </button>

            <button onClick={() => localParticipant?.setMicrophoneEnabled(!localParticipant.isMicrophoneEnabled)} style={btnReset}>
