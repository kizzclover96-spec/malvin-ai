...
function VideoStage({ onDisconnect, userEmail }: { onDisconnect: () => void, userEmail: string }) {
  ...
  // Fixed Storage Key Guard
  const storageKey = `malvin_session_${(userEmail || 'guest').replace(/[^a-zA-Z0-9]/g, '_')}`;
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, backgroundColor: '#000', color: '#fff', overflow: 'hidden', fontFamily: 'sans-serif' }}>
      ...
      {backgroundImage && !screenTrack && !cameraTrack && <div style={{ position: 'absolute', inset: 0, backgroundImage: `url(${backgroundImage})`, backgroundSize: 'cover', backgroundPosition: 'center', filter: `blur(${bgBlur}px)`, transform: 'scale(1.1)', zIndex: 2 }} />}
      ...
      <div style={{ position: 'absolute', top: 0, left: isSettingsOpen ? 0 : '-320px', display: isSettingsOpen ? 'flex' : 'none', width: '280px', height: '100%', backgroundColor: 'rgba(10,10,10,0.98)', borderRight: `1px solid ${neonBlue}44`, zIndex: 200, transition: 'left 0.4s ease', flexDirection: 'column', boxShadow: '20px 0 50px rgba(0,0,0,0.5)', backdropFilter: 'blur(10px)' }}>
        <div style={{ display: 'flex', marginTop: '70px', padding: '0 20px', borderBottom: '1px solid #222' }}>
           {['notes', 'appearance', 'system'].map((t) => <div key={t} onClick={() => setActiveTab(t as any)} style={{ flex: 1, padding: '10px 0', textAlign: 'center', cursor: 'pointer', fontSize: '11px', fontWeight: 'bold', color: activeTab === t ? neonBlue : '#444', borderBottom: activeTab === t ? `2px solid ${neonBlue}` : 'none', textTransform: 'uppercase' }}>{t}</div>)}
        </div>
          ...
          {activeTab === 'system' && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize: '13px', color: neonBlue, marginTop: '4px' }}>{userEmail || 'Guest'}</div>
                </div>
                <button onClick={() => setIsDashboardOpen(true)} style={{ background: 'rgba(191, 0, 255, 0.05)', border: `1px solid ${neonPurple}`, color: neonPurple, padding: '12px', borderRadius: '8px', cursor: 'pointer', fontSize: '12px', fontWeight: 'bold', textTransform: 'uppercase', animation: 'dashboardGlow 3s infinite' }}>Dashboard</button>
