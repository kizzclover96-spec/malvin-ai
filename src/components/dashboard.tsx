...
const dashboard = (props) => {
    ...
    const visionInterval = useRef<NodeJS.Timeout | null>(null);
    ...
    const startVision = async () => {
        try {
            ...
            // Clear any existing timer using .current
            if (visionInterval.current) clearInterval(visionInterval.current);

            // Store the new timer in .current
    ...
    return (
        <>
            ...
            overflowX: 'hidden'
                ...
                {/* --- HEADER AREA --- */}
                <div style={{ 
                    width: '100%', maxWidth: '1400px', margin: '0 auto', padding: '20px',
                    ...
                    <div style={{ 
                        ...
                        background: '#111', padding: '6px', borderRadius: '40px', display: 'flex', 
                        gap: '5px', border: '1px solid #222'
                    }}>
                        {navItems.map(item => (
                            <div 
                            key={item} onClick={() => setActiveTab(item)}
                            style={{
                                padding: '10px 24px', borderRadius: '30px', fontSize: '13px', fontWeight: 600, cursor: 'pointer',
                ...
                <div style={{ flex: 1, padding: '0 20px 20px 20px', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
                    {activeTab === 'Chats' ? (
                      <Chats userBrand={userBrand} onBack={() => setActiveTab('Invoices')} brandId={auth.currentUser?.uid} />
                    ) : activeTab === 'Ads' ? (
                      <AdsManager userBrand={userBrand} onBack={() => setActiveTab('Invoices')} brandName={userBrand?.name} />
                    ) : activeTab === 'Catalog' ? (
                      <Catalog userBrand={userBrand} brandName={userBrand?.name} onBack={() => setActiveTab('Invoices')} />
                    ) : activeTab === 'Payments' ? (
                      <Payments userBrand={userBrand} brandName={userBrand?.name} onBack={() => setActiveTab('Invoices')} />
                    ) : (
