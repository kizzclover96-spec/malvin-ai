...
const Premium: React.FC<PremiumProps> = ({ onBack }) => {
  ...
  const handleBack = () => {
    // 1. If an explicit callback prop was passed, run it
    ...
    // 2. If running inside an iframe, tell the top-level parent window to go back
    if (window.parent && window.parent !== window) {
      window.parent.postMessage({ type: "GO_BACK" }, "*");
      return;
    }

    // 3. Fallback to native browser history back
    window.history.back();
  };
