...
const AdsManager = ({ userBrand }: any) => {
    ...
    const activeCampaigns = campaigns.filter(
        c => c.status === "Running" || c.status === "Approved"
    ).length;
    ...
    const grandTotal = totalInvestment + neuralFee;
    ...
    return (
        <>
            ...
            <AdReachEngine />
            <div className="ads-container" style={{ padding: '20px', color: 'white' }}>
                <div className="header-flex" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: '30px' }}>
                    <div>
                        <h1 style={{ fontSize: '32px', fontWeight: 700, margin: 0 }}>Ad_Deploy</h1>
                        <p style={{ opacity: 0.5, marginTop: '4px' }}>Amplify {userBrand?.name} | Balance: <span style={{color: '#C5FF41'}}>€{currentBalance.toLocaleString()}</span></p>
                    </div>
                    <button onClick={() => setShowModal(true)} style={deployBtnStyle}>Launch New Campaign</button>
                </div>    

                {/* --- ANALYTICS OVERVIEW --- */}
                <div className="stats-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '15px', marginBottom: '30px' }}>
                    <div style={statBox}>
                ...
                {/* --- CAMPAIGN LIST (MODIFIED FOR SCROLLABLE TABLE ON MOBILE) --- */}
                <div style={{ 
                ...
                {showModal && (
                    <div style={overlayStyle}>
                        <div className="glass-modal" style={glassModal}>
                            ...
                            <div className="form-group-split" style={{ display: 'flex', gap: '15px' }}>
                                <div style={{ flex: 1 }}>
                            ...
                            <input
                                style={inputStyle}
                                placeholder="(paste your ad link https://yourchat.com)"
                                value={newAd.website}
                                onChange={(e) =>
                                    setNewAd({
                                        ...newAd,
                            ...
                            {/* --- PRICE BREAKDOWN --- */}
                            <div style={{ 
                                padding: '16px', background: 'rgba(197, 255, 65, 0.05)', 
...
const statBox = { background: 'rgba(255,255,255,0.03)', padding: '20px', borderRadius: '16px', border: '1px solid rgba(255,255,255,0.08)' };
const statLabel = { fontSize: '11px', opacity: 0.4, textTransform: 'uppercase' as 'uppercase', letterSpacing: '1px' };
const statValue = { fontSize: '24px', fontWeight: 700, marginTop: '8px', color: '#C5FF41' };
const thStyle = { padding: '16px 20px', opacity: 0.4, fontSize: '11px', textTransform: 'uppercase' as 'uppercase' };
...
const inputStyle = { width: '100%', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', padding: '12px', borderRadius: '12px', color: 'white', marginBottom: '16px', outline: 'none', boxSizing: 'border-box' as 'border-box' };
const labelStyle = { fontSize: '10px', opacity: 0.4, textTransform: 'uppercase' as 'uppercase', marginBottom: '6px', display: 'block' };
