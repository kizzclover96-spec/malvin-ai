...
const MarketFront = ({ brandId: propBrandId, userBrand, brandName }: { brandId?: string, userBrand?: any, brandName?: string }) => {
    ...
    const getConversationId = (brandId?: string) => {
        const currentUser = auth.currentUser?.uid;
        if (!brandId || !currentUser) return null;

        return [currentUser, brandId].sort().join("_");
    };
    ...
    useEffect(() => {
        ...
        // 🧹 CLEANUP: Erase user auth from memory on unmount/close
        return () => {
    ...
    // Use `activeUserUid || auth.currentUser?.uid` whenever performing transactions or writing to Firebase
    const currentUserId = activeUserUid || auth.currentUser?.uid;
    ...
    const conversationId = getConversationId(brandId);
